...
/*
 * Bitmap of monitor events currently enabled. Upstream provides
 * resctrl_enable_mon_event() / resctrl_is_mon_event_enabled() in
 * fs/resctrl/monitor.c; until that lands here, keep file-local
 * equivalents so CBQRI can publish what events it supports.
 */
static unsigned long cbqri_enabled_mon_events;

static inline void resctrl_enable_mon_event(enum resctrl_event_id evt,
					    bool configurable, u32 cfg, void *unused)
{
	set_bit(evt, &cbqri_enabled_mon_events);
}

static inline bool resctrl_is_mon_event_enabled(enum resctrl_event_id evt)
{
	return test_bit(evt, &cbqri_enabled_mon_events);
}
...
int resctrl_arch_rmid_read(struct rdt_resource *r, struct rdt_mon_domain *mon_d,
			   u32 closid, u32 rmid, enum resctrl_event_id eventid,
			   u64 *val, void *arch_mon_ctx)
{
	struct rdt_domain_hdr *hdr = &mon_d->hdr;
	struct cbqri_resctrl_dom *hw_dom;
	struct cbqri_controller *ctrl;
				      ...
				      struct rdt_resource *res, unsigned int cpu)
{
	...
	err = resctrl_online_mon_domain(res, mon_dom);
	if (err)
		goto err_listdel;
...
err_offline:
	...
	resctrl_offline_mon_domain(res, mon_dom);
err_listdel:
