	...
	if (ret.error) {
		...
		case SBI_ERR_INVALID_ADDRESS:
			pr_warn("%s: Invalid address parameter (%ld)\n",
				__func__, (unsigned long)ret.error);
			rc = -EINVAL;
			break;
...
static int arch_smp_teardown_sbi_shmem(unsigned int cpu)
{
	...
	if (ret.error) {
		...
		case SBI_ERR_INVALID_PARAM:
		case SBI_ERR_INVALID_ADDRESS:
			pr_err("%s: Invalid address parameter (%ld)\n",
			       __func__, (unsigned long)ret.error);
			break;
...
void arch_enable_hw_breakpoint(struct perf_event *event)
{
	...
	if (ret.error) {
		pr_warn("%s: Failed to enable trigger %d\n", __func__, i);
		return;
	}
}
...
void arch_disable_hw_breakpoint(struct perf_event *event)
{
	...
	if (ret.error) {
		pr_warn("%s: Failed to disable trigger %d.\n", __func__, i);
		return;
	}
}
