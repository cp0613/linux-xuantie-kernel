...
int kvm_arch_vcpu_create(struct kvm_vcpu *vcpu)
{
	...
	if (kvm_riscv_vcpu_alloc_m_context(vcpu, cntx))
		return -ENOMEM;
...
void kvm_arch_vcpu_destroy(struct kvm_vcpu *vcpu)
{
	...
	/* Free matrix context space for host and guest kernel */
	kvm_riscv_vcpu_free_m_context(vcpu);
}
...
void kvm_arch_vcpu_load(struct kvm_vcpu *vcpu, int cpu)
{
	...
	kvm_riscv_vcpu_guest_vector_restore(&vcpu->arch.guest_context,
					    vcpu->arch.isa);
	kvm_riscv_vcpu_host_m_save(&vcpu->arch.host_context);
	kvm_riscv_vcpu_guest_m_restore(&vcpu->arch.guest_context,
					    vcpu->arch.isa);
...
void kvm_arch_vcpu_put(struct kvm_vcpu *vcpu)
{
	...
	kvm_riscv_vcpu_guest_m_save(&vcpu->arch.guest_context,
					 vcpu->arch.isa);
	kvm_riscv_vcpu_host_m_restore(&vcpu->arch.host_context);
